...
const admin = require('firebase-admin'); // firebase Admin 초기화 
...
router.get('/List', (req, res, next) => { 
  firebaseAdmin.database().ref('posts').orderByKey().once('value', snapshot => { 
    var rows = []; 
    snapshot.forEach(childSnapshot => { 
      var childData = childSnapshot.val(); 
      childData.postsdate = dateFormat(childData.postsdate, "yyyy-mm-dd"); 
      rows.push(childData); }); res.render('rPosts/List', {rows: rows}); }); }); 

 router.get('/Read', (req, res, next) => { 
   firebaseAdmin.database().ref('posts/' + req.query.postsno).once('value', snapshot => { 
  ...
  router.post('/Save', (req, res, next) => { 
    ...
    firebaseAdmin.database()
      .ref('posts/' + req.body.postsno)
      .set( req.body, err => {
        if(err) console.log(err)
        else { console.log(req.body.postsno + " Upload successfully") } 
      }) 
      res.redirect('List'); }); 
